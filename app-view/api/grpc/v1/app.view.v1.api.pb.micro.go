// Code generated by protoc-gen-micro. DO NOT EDIT.
// source: app.view.v1.api.proto

package v1

import (
	fmt "fmt"
	proto "github.com/golang/protobuf/proto"
	math "math"
)

import (
	context "context"
	api "github.com/micro/go-micro/v2/api"
	client "github.com/micro/go-micro/v2/client"
	server "github.com/micro/go-micro/v2/server"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion3 // please upgrade the proto package

// Reference imports to suppress errors if they are not otherwise used.
var _ api.Endpoint
var _ context.Context
var _ client.Option
var _ server.Option

// Api Endpoints for AppviewService service

func NewAppviewServiceEndpoints() []*api.Endpoint {
	return []*api.Endpoint{}
}

// Client API for AppviewService service

type AppviewService interface {
	ListHomeMo(ctx context.Context, in *ListHomeMoRequest, opts ...client.CallOption) (*ListHomeMoResponse, error)
}

type appviewService struct {
	c    client.Client
	name string
}

func NewAppviewService(name string, c client.Client) AppviewService {
	return &appviewService{
		c:    c,
		name: name,
	}
}

func (c *appviewService) ListHomeMo(ctx context.Context, in *ListHomeMoRequest, opts ...client.CallOption) (*ListHomeMoResponse, error) {
	req := c.c.NewRequest(c.name, "AppviewService.ListHomeMo", in)
	out := new(ListHomeMoResponse)
	err := c.c.Call(ctx, req, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

// Server API for AppviewService service

type AppviewServiceHandler interface {
	ListHomeMo(context.Context, *ListHomeMoRequest, *ListHomeMoResponse) error
}

func RegisterAppviewServiceHandler(s server.Server, hdlr AppviewServiceHandler, opts ...server.HandlerOption) error {
	type appviewService interface {
		ListHomeMo(ctx context.Context, in *ListHomeMoRequest, out *ListHomeMoResponse) error
	}
	type AppviewService struct {
		appviewService
	}
	h := &appviewServiceHandler{hdlr}
	return s.Handle(s.NewHandler(&AppviewService{h}, opts...))
}

type appviewServiceHandler struct {
	AppviewServiceHandler
}

func (h *appviewServiceHandler) ListHomeMo(ctx context.Context, in *ListHomeMoRequest, out *ListHomeMoResponse) error {
	return h.AppviewServiceHandler.ListHomeMo(ctx, in, out)
}
