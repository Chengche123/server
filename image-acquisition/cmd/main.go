package main

import (
	"image-acquisition-service/conf"
	"image-acquisition-service/dao"
	mserver "image-acquisition-service/server/grpc"
	mservice "image-acquisition-service/service"
	"os"
	"os/signal"
	zlog "share/log/zap"
	"syscall"
	"time"

	"auth-service/token"

	"go.uber.org/zap"
)

func main() {
	tokenGenerator, err := token.NewJWTTokenGen([]byte(privateKey), "image-acquisition/auth")
	if err != nil {
		zlog.Logger.Error("", zap.Error(err))
		return
	}

	userRepository, err := dao.NewUserRepository(conf.DSN)
	if err != nil {
		zlog.Logger.Error("", zap.Error(err))
		return
	}
	defer userRepository.Close()

	service, err := mserver.NewImageAcquisitionServer(&mservice.UserService{
		UserRepository: userRepository,
		TokenGenerator: tokenGenerator,
	})
	if err != nil {
		zlog.Logger.Error("", zap.Error(err))
		return
	}

	go func() {
		err = service.Run()
		if err != nil {
			zlog.Logger.Error("", zap.Error(err))
			return
		}
	}()

	termChan := make(chan os.Signal, 1)
	signal.Notify(termChan, os.Interrupt, syscall.SIGTERM)
	<-termChan

	closed := make(chan struct{})
	go func() {
		_ = userRepository.Close()
		_ = service.Server().Stop()

		close(closed)
	}()

	select {
	case <-closed:
		zlog.Logger.Info("graceful shutdown")
	case <-time.After(3 * time.Second):
		zlog.Logger.Error("shutdown timeout")
	}
}

const privateKey = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`
